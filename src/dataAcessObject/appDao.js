...
const findAllPodcast = async()=>{
    ...
    try {
        ...
        const podcasts = await db.query(`SELECT Podcasts.id,  Podcasts.name as title, Users.names as artist, 'podcast' as album, Podcasts.cover as artwork, Podcasts.url,  Podcasts.owner, Podcasts.category, Podcasts.description, Podcasts.price, Podcasts.likes, Podcasts.views, Podcasts.isFree, Podcasts.createdAt, Podcasts.updatedAt FROM Users RIGHT JOIN Podcasts ON Users.email = Podcasts.owner ORDER BY Podcasts.createdAt DESC LIMIT 40;`, QueryTypes.SELECT);
        return podcasts[0]
    } catch (error) {
        throw new Error(error.message)
    }
